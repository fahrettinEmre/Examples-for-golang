...
//TODOS:
/*
gametable'nin dışına çıkamazlar.
tanklar negatif ve pozitif yönde hareket etmeliler.
tankların ammo sayısı kadar ateş etmeleri gerekir.
Tank station eklenecek , 2 turn hareket etmeden duracak ancak healti ve ammınationu fullenecek,
Bir tank isabetli atış sonrası damage gücü artacaktır.
Tanklara mazot fieldi eklenecek, gitti yol kadar x ve y düzleminde mazotu azalacak.
Mavi ve kırmızı takımlar olacak maviler kırmızıları kırmızılar mavileri vurmaya çalışacak.
*/
...
func (t *Tank) GetingDamage(d int) {
	if t.healt > 0 {
		t.healt -= d
	}
	if t.healt <= 0 {
